...
export async function postTarefa(tarefas: string) {
    try {
        const resposta = await fetch(env.api_url_tarefas, {
            method: "POST",
            headers: {
        ...
        console.error(
            `Erro de servidor ao enviar tarefa ao database: ${err}`
        );
        throw err;
    }
}
...
export async function getTarefa() {
    try {
        ...
        console.error(
            `Erro de servidor ao buscar tarefas do database: ${err}`
        );
        throw err;
    }
}
...
export async function deleteTarefa(id: number) {
    try {
        const resposta = await fetch(`${env.api_url_tarefas}/${id}`, {
            method: "DELETE"
        });
        ...
        console.error(
            `Erro de servidor ao deletar tarefa do banco de dados: ${err}`
        );
        throw err;
    }
}
...
export async function putEditarTarefa(id: number, tarefas: string) {
    try {
        ...
        console.error(
            `Erro no servidor ao editar tarefa no banco de dados: ${err}`
        );
        throw err;
    }
}

export async function putTarefaConcluida(
    id: number,
    concluida: boolean
) {
    try {
        const resposta = await fetch(
            `${env.api_url_tarefas}/${id}/concluida`,
            {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json"
        ...
        console.error(
            `Erro no servidor ao atualizar tarefa concluida no banco de dados: ${err}`
        );
        throw err;
    }
}